...
CREATE DATABASE IF NOT EXISTS HOL_DEALER360_MASTER
    COMMENT = 'Dealer 360 HOL lab template — hydrated from git seed data, cloned per attendee';

USE DATABASE HOL_DEALER360_MASTER;
...
CREATE OR REPLACE GIT REPOSITORY HOL_DEALER360_MASTER.CORE.HOL_GIT_REPO
    API_INTEGRATION = HOL_GIT_API_INTEGRATION
    ORIGIN = 'https://github.com/evolvconsulting/coco_semantic_modeling_hol.git';

ALTER GIT REPOSITORY HOL_DEALER360_MASTER.CORE.HOL_GIT_REPO FETCH;
...
CREATE OR REPLACE STAGE HOL_DEALER360_MASTER.CORE.HOL_SEED_STAGE
    FILE_FORMAT = (TYPE = CSV SKIP_HEADER = 1 FIELD_OPTIONALLY_ENCLOSED_BY = '"');

COPY FILES INTO @HOL_DEALER360_MASTER.CORE.HOL_SEED_STAGE
    FROM @HOL_DEALER360_MASTER.CORE.HOL_GIT_REPO/branches/main/dealer_360_seed_data/;

COPY INTO DEALER_MASTER
    FROM @HOL_DEALER360_MASTER.CORE.HOL_SEED_STAGE/dealer_master.csv
    FILE_FORMAT = (TYPE = CSV SKIP_HEADER = 1 FIELD_OPTIONALLY_ENCLOSED_BY = '"');

COPY INTO APPLICATION_EVENTS
    FROM @HOL_DEALER360_MASTER.CORE.HOL_SEED_STAGE/application_events.csv
    FILE_FORMAT = (TYPE = CSV SKIP_HEADER = 1 FIELD_OPTIONALLY_ENCLOSED_BY = '"');

COPY INTO DEALER_PERFORMANCE
    FROM @HOL_DEALER360_MASTER.CORE.HOL_SEED_STAGE/dealer_performance.csv
    FILE_FORMAT = (TYPE = CSV SKIP_HEADER = 1 FIELD_OPTIONALLY_ENCLOSED_BY = '"');

COPY INTO FUNDING_EVENTS
    FROM @HOL_DEALER360_MASTER.CORE.HOL_SEED_STAGE/funding_events.csv
    FILE_FORMAT = (TYPE = CSV SKIP_HEADER = 1 FIELD_OPTIONALLY_ENCLOSED_BY = '"');

COPY INTO SERVICING_EVENTS
    FROM @HOL_DEALER360_MASTER.CORE.HOL_SEED_STAGE/servicing_events.csv
    FILE_FORMAT = (TYPE = CSV SKIP_HEADER = 1 FIELD_OPTIONALLY_ENCLOSED_BY = '"');
...
BEGIN
    FOR i IN 1 TO :num_users DO
        LET suffix   VARCHAR := LPAD(i::VARCHAR, 2, '0');
        LET db_name  VARCHAR := 'HOL_DEALER360_' || :suffix;

        EXECUTE IMMEDIATE
            'CREATE DATABASE IF NOT EXISTS ' || :db_name ||
            ' CLONE HOL_DEALER360_MASTER';
    END FOR;
