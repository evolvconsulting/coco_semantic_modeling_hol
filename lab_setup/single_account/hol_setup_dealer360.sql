...
--
-- Objects created: HOL_DEALER360_WH, COCO_SDLC_HOL_DEALER.CORE (5 tables, seeded),
...
CREATE DATABASE IF NOT EXISTS COCO_SDLC_HOL_DEALER
    COMMENT = 'Dealer 360 HOL lab database — hydrated from git seed data';

USE DATABASE COCO_SDLC_HOL_DEALER;
CREATE SCHEMA IF NOT EXISTS CORE;
USE SCHEMA CORE;

CREATE OR REPLACE GIT REPOSITORY COCO_SDLC_HOL_DEALER.CORE.HOL_GIT_REPO
    API_INTEGRATION = HOL_GIT_API_INTEGRATION
    ORIGIN = 'https://github.com/evolvconsulting/coco_semantic_modeling_hol.git';

ALTER GIT REPOSITORY COCO_SDLC_HOL_DEALER.CORE.HOL_GIT_REPO FETCH;
...
CREATE OR REPLACE STAGE COCO_SDLC_HOL_DEALER.CORE.HOL_SEED_STAGE
    FILE_FORMAT = (TYPE = CSV SKIP_HEADER = 1 FIELD_OPTIONALLY_ENCLOSED_BY = '"');

COPY FILES INTO @COCO_SDLC_HOL_DEALER.CORE.HOL_SEED_STAGE
    FROM @COCO_SDLC_HOL_DEALER.CORE.HOL_GIT_REPO/branches/main/dealer_360_seed_data/;

COPY INTO DEALER_MASTER
    FROM @COCO_SDLC_HOL_DEALER.CORE.HOL_SEED_STAGE/dealer_master.csv
    FILE_FORMAT = (TYPE = CSV SKIP_HEADER = 1 FIELD_OPTIONALLY_ENCLOSED_BY = '"');

COPY INTO APPLICATION_EVENTS
    FROM @COCO_SDLC_HOL_DEALER.CORE.HOL_SEED_STAGE/application_events.csv
    FILE_FORMAT = (TYPE = CSV SKIP_HEADER = 1 FIELD_OPTIONALLY_ENCLOSED_BY = '"');

COPY INTO DEALER_PERFORMANCE
    FROM @COCO_SDLC_HOL_DEALER.CORE.HOL_SEED_STAGE/dealer_performance.csv
    FILE_FORMAT = (TYPE = CSV SKIP_HEADER = 1 FIELD_OPTIONALLY_ENCLOSED_BY = '"');

COPY INTO FUNDING_EVENTS
    FROM @COCO_SDLC_HOL_DEALER.CORE.HOL_SEED_STAGE/funding_events.csv
    FILE_FORMAT = (TYPE = CSV SKIP_HEADER = 1 FIELD_OPTIONALLY_ENCLOSED_BY = '"');

COPY INTO SERVICING_EVENTS
    FROM @COCO_SDLC_HOL_DEALER.CORE.HOL_SEED_STAGE/servicing_events.csv
    FILE_FORMAT = (TYPE = CSV SKIP_HEADER = 1 FIELD_OPTIONALLY_ENCLOSED_BY = '"');
